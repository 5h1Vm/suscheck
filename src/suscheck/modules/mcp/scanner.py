...
def _rules_path() -> Path:
    # Use relative path from this file to locate rules robustly
    p = Path(__file__).resolve().parent / "rules" / "mcp.toml"
    if p.exists():
        return p
    # Fallback for dev/test environments
    return Path.cwd() / "src" / "suscheck" / "modules" / "mcp" / "rules" / "mcp.toml"
